...
describe('JUnitReporterFactoryImpl', () => {
  const junixXmlReaderMock = jest.spyOn(JUnitXmlReader.prototype, 'safeParse').mockResolvedValue({
    testsuites: {}
  })
  ...
  const context = {
    ...
    it.each(testCases)('%s', async ({ input, expected }) => {
      const factory = new JUnitReporterFactory(new JUnitXmlReader(fs.promises.readFile))
      const result = await factory.fromXml(
        context,
        input,
        directory,
        fileName
